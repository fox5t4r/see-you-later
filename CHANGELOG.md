...
### Fixed
- **Watch Later 제목 오염**: "시청함4:24 지금 재생 중" 같은 YouTube 플레이어 오버레이 텍스트가 제목으로 캡처되던 문제 수정
- DOM 파싱에서 `title` 속성 → `aria-label` → `#video-title` textContent 순으로 깨끗한 제목 추출
- 오버레이 텍스트 패턴("시청함", "지금 재생 중", 타임스탬프만 있는 텍스트 등) 자동 필터링
...
### Fixed
- **[치명적] Watch Later 내보내기 실패**: 수동 동기화 시 `watchLaterAutoExport` 설정과 무관하게 자격증명이 있는 모든 서비스로 내보내기 (기존: 'none'이면 어디에도 전송 안 됨)
- **[치명적] 거짓 알림**: 내보내기가 실패/미설정이어도 "전송되었습니다"로 표시되던 버그 수정. 이제 성공/실패/미설정을 정확히 표시
- **내보내기 에러 무시**: `.catch(() => {})` 제거, 실패 시 콘솔 에러 로그 기록 및 알림에 실패 건수 표시

### Changed
- README: 추천도 평가 기준표 추가, 자동 요약 참고사항 (백그라운드 동작, 중복 확인 없음, 지연 안내) 추가
- 버전 0.2.4 → 0.2.5
