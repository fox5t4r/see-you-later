...
### Fixed
- **"나중에 볼 동영상" 파싱 전면 재구현**: 3가지 추출 전략 (DOM 파싱 → ytInitialData → script 태그), 최대 3회 재시도, 기존 탭 자동 새로고침, 대기 시간 증가 (1.5초 → 3초)
- **추천도 별점 기준 대폭 강화**: 기존 4~5점 편향 해소. 대부분 2~3점, 4점 이상은 상위 20% 콘텐츠에만 부여
- **프롬프트 예시 score 하향**: 기본 예시 3~4점 → 2점으로 변경하여 AI 편향 제거

### Changed
- 설정 화면 무료 한도 안내에서 "(Gemini 2.5 Pro 기준)" 문구 제거
- 버전 0.2.3 → 0.2.4
